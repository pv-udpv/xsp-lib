...
References:
    - VAST 4.2 Specification: Configuration and initialization patterns
"""
...
class VastChainConfigLoader:
    ...
    Example:
        >>> loader = VastChainConfigLoader()
        >>> resolvers = loader.load("config/vast_chains.yaml", transport)
        >>> result = await resolvers["default"].resolve(params={"w": "640"})
    """
    ...
    def load(cls, config_path: str | Path, transport: Transport) -> dict[str, VastChainResolver]:
        ...
        Example:
            ...
            >>> resolvers = VastChainConfigLoader.load("vast_chains.yaml", transport)
    ...
    def _create_upstreams(
        ...
        Args:
            upstreams_config: Upstreams configuration dictionary
            transport: Transport instance

        Returns:
            Dictionary of upstream name -> VastUpstream instance
        ...
        # Parse nested upstream structure
        # Format: upstreams.vast.primary.endpoint
    ...
    def _create_resolvers(
        ...
        Args:
            chains_config: Chains configuration dictionary
            upstreams: Dictionary of available upstream instances

        Returns:
            Dictionary of chain name -> VastChainResolver instance

        Raises:
            ValueError: If configuration is invalid or references missing upstreams
        """
        ...
        for chain_name, chain_config in chains_config.items():
            ...
            # Build upstream dict (primary + fallbacks)
    ...
    def _expand_env_vars(cls, value: str) -> str:
        ...
        Example:
            ...
            >>> VastChainConfigLoader._expand_env_vars("https://${HOST}/vast")
            'https://example.com/vast'
            >>> VastChainConfigLoader._expand_env_vars("${PORT:-8080}")
            '8080'
        """
